...
function MonDKP_RestoreFilterOptions()
    -- restores default filter selections
...
function MonDKP:Toggle()
    -- toggles IsShown() state of MonDKP.UIConfig, the entire addon window
    ...
    if core.BiddingWindow then
        core.BiddingWindow:SetFrameLevel(6)
    end
    if core.ModesWindow then
        core.ModesWindow:SetFrameLevel(2)
    end
...
function MonDKP:FilterDKPTable(sort, reset)
    -- filters core.WorkingTable based on classes in classFiltered table. core.currentSort should be used in most cases
    local parentTable;
    ...
    if #core.WorkingTable == 0 then
        -- removes all filter settings if the filter combination results in an empty table
...
function MonDKP:SortDKPTable(id, reset)
    -- reorganizes core.WorkingTable based on id passed. Avail IDs are "class", "player" and "dkp"
    ...
    elseif id == "spec" then
        -- doesn't allow "spec" to be sorted.
        DKPTable_Update()
        return ;
    else
    ...
    if reset and reset ~= "Clear" then
        -- reset is useful for check boxes when you don't want it repeatedly reversing the sort
...
function MonDKP:CreateMenu()
    ...
    MonDKP.UIConfig:SetScript("OnMouseDown", function(self)
        ...
        if core.BiddingWindow then
            core.BiddingWindow:SetFrameLevel(2)
        end
    end)
    ...
    for k, v in pairs(SortButtons) do
        ...
        if v.Id == "class" then
            ...
            v:SetScript("OnClick", function(self)
                MonDKP:SortDKPTable(self.Id, "Clear")
            end)
        end
    end
    ...
    MonDKP.UIConfig.search:SetScript("OnEscapePressed", function(self)
        -- clears text and focus on esc
    ...
    MonDKP.UIConfig.search:SetScript("OnEnterPressed", function(self)
        -- clears text and focus on enter
        self:ClearFocus()
    end)
    MonDKP.UIConfig.search:SetScript("OnTabPressed", function(self)
        -- clears text and focus on tab
        self:ClearFocus()
    end)
