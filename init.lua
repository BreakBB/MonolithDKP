...
function MonDKP_OnEvent(self, event, arg1, ...)
    ...
    elseif event == "UPDATE_INSTANCE_INFO" then
        ...
        if not MonDKP_DB.Lockouts then
            MonDKP_DB.Lockouts = { Three = 0, Five = 1570032000, Seven = 1569945600 }
        end

        for i = 1, num do
            -- corrects reset timestamp for any raids where an active lockout exists
        ...
        while MonDKP_DB.Lockouts.Three < time() do
            MonDKP_DB.Lockouts.Three = MonDKP_DB.Lockouts.Three + 259200
        end
        while MonDKP_DB.Lockouts.Five < time() do
            MonDKP_DB.Lockouts.Five = MonDKP_DB.Lockouts.Five + 432000
        end
        while MonDKP_DB.Lockouts.Seven < time() do
            MonDKP_DB.Lockouts.Seven = MonDKP_DB.Lockouts.Seven + 604800
        end
        ...
        for k, v in pairs(MonDKP_DB.Lockouts) do
            ...
            elseif k == "Five" then
                raidString = "Onyxia"
            elseif k == "Seven" then
                raidString = "MC, BWL, AQ40"
            end

            if k ~= "Three" then
                -- remove when three day raid lockouts are added
    ...
    elseif event == "LOOT_OPENED" then
        ...
        if core.IsOfficer then
            if not IsInRaid() and arg1 == false then
                -- only fires hook when autoloot is not active if not in a raid to prevent nil value error
