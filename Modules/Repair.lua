...
local function ConsolidateTables(keepDKP)
    table.sort(ConsolidatedTable, function(a, b)
        -- inverts tables; oldest to newest
    ...
    ValidateTimer:SetScript("OnUpdate", function(self, elapsed)
        ...
        if timer > 0.01 and i <= #ConsolidatedTable and not processing then
            ...
            elseif ConsolidatedTable[i].reason then
                ...
                if strfind(ConsolidatedTable[i].dkp, "%-%d*%.?%d+%%") then
                    -- is a decay, calculate new values
        ...
        elseif i > #ConsolidatedTable then
            ...
            for i = 1, #DKPTableTemp do
                -- finds who had history but was deleted; adds them to archive if so
    ...
    ValidateTimer:SetScript("OnUpdate", function(self, elapsed)
        ...
        if timer > 0.01 and i <= #MonDKP_DKPHistory and not processing then
            ...
            if MonDKP_DKPHistory[i].deletes or MonDKP_DKPHistory[i].deletedby or MonDKP_DKPHistory[i].reason == "Migration Correction" then
                -- removes deleted entries/Migration Correction
...
function MonDKP:RepairTables(keepDKP)
    -- Repair starts
