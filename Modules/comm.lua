...
function MonDKP:ValidateSender(sender)
    -- returns true if "sender" has permission to write officer notes. false if not or not found.
    ...
    if #MonDKP_Whitelist > 0 then
        -- if a whitelist exists, checks that rather than officer note permissions
    ...
    if prefix then
        ...
        if prefix == "MonDKPQuery" then
            ...
            if sender ~= UnitName("player") and message ~= "start" then
                -- logs seed. Used to determine if the officer has entries required.
                ...
                if MonDKP:ValidateSender(off1) and MonDKP:ValidateSender(off2) and tonumber(date1) > MonDKP_DB.defaults.installed210 and tonumber(date2) > MonDKP_DB.defaults.installed210 then
                    -- send only if posting officer validates and the post was made after 2.1s installation
            ...
            -- talents check
            local TalTrees = {};
            table.insert(TalTrees, { GetTalentTabInfo(1) });
            table.insert(TalTrees, { GetTalentTabInfo(2) });
            table.insert(TalTrees, { GetTalentTabInfo(3) });
        ...
        elseif prefix == "MonDKPBuild" and sender ~= UnitName("player") then
            ...
            if LastVerCheck > 900 then
                -- limits the Out of Date message from firing more than every 15 minutes
            ...
            if tonumber(message) < core.BuildNumber then
                -- returns build number if receiving party has a newer version
        ...
        if MonDKP:ValidateSender(sender) then
            ...
            elseif (prefix == "MonDKPCommand") then
                ...
                if sender ~= UnitName("player") then
                    ...
                    elseif command == "BidInfo" then
                        ...
                        if MonDKP_DB.defaults.AutoOpenBid and not core.BidInterface:IsShown() then
            ...
            if (sender ~= UnitName("player")) then
                ...
                then
                    ...
                    if success then
                        if prefix == "MonDKPAllTabs" then
                            -- receives full table broadcast
                        ...
                        elseif prefix == "MonDKPMerge" then
                            for i = 1, #deserialized.DKP do
                                ...
                                if not search and ((MonDKP_Archive.DKPMeta and MonDKP_Archive.DKPMeta < deserialized.DKP[i].date) or (not MonDKP_Archive.DKPMeta)) then
                                    -- prevents adding entry if this entry has already been archived
                                    ...
                                    if deserialized.DKP[i].deletes then
                                        -- adds deletedby field to entry if the received table is a delete entry
                                            ...
                                            if strfind(deserialized.DKP[i].dkp, "%-%d*%.?%d+%%") then
                                                ...
                                                if findEntry then
                                                    ...
                                                    if (tonumber(deserialized.DKP[i].dkp) > 0 and not deserialized.DKP[i].deletes) or (tonumber(deserialized.DKP[i].dkp) < 0 and deserialized.DKP[i].deletes) then
                                                        -- adjust lifetime if it's a DKP gain or deleting a DKP gain
                            ...
                            for i = 1, #deserialized.Loot do
                                ...
                                if not search and ((MonDKP_Archive.LootMeta and MonDKP_Archive.LootMeta < deserialized.DKP[i].date) or (not MonDKP_Archive.LootMeta)) then
                                    -- prevents adding entry if this entry has already been archived
                            ...
                            for i = 1, #deserialized do
                                ...
                                if search and deserialized[i].deleted and deserialized[i].deleted ~= "Recovered" then
                                    if (MonDKP_Archive[deserialized[i].player] and MonDKP_Archive[deserialized[i].player].edited < deserialized[i].edited) or not MonDKP_Archive[deserialized[i].player] then
                                        ...
                                        if not MonDKP_Archive[deserialized[i].player] then
                                            -- creates/adds to archive entry for user
                        ...
                        elseif prefix == "MonDKPDelSync" then
                            ...
                            if strfind(deserialized.dkp, "%-%d*%.?%d+%%") then
                                -- determines if it's a mass decay
                        ...
                        elseif prefix == "MonDKPWhitelist" and MonDKP:GetGuildRankIndex(UnitName("player")) > 1 then
                            -- only applies if not GM
...
function MonDKP.Sync:SendData(prefix, data, target)
    ...
    if data == nil or data == "" then
        data = " "
    end -- just in case, to prevent disconnects due to empty/nil string AddonMessages
    ...
    if IsInGuild() and core.IsOfficer then
        ...
        -- encoded
        if (prefix == "MonDKPZSumBank" or prefix == "MonDKPBossLoot" or prefix == "MonDKPBidShare") then
            -- Zero Sum bank/loot table/bid table data and bid submissions. Keep to raid.
            MonDKP.Sync:SendCommMessage(prefix, packet, "RAID")
            return ;
        end
