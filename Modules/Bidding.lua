...
local function Roll_OnEvent(self, event, arg1, ...)
    if event == "CHAT_MSG_SYSTEM" and core.BidInProgress then
        ...
        end  -- corrects roll pattern for german clients
...
function MonDKP_CHAT_MSG_WHISPER(text, ...)
    ...
    if string.find(name, "-") then
        -- finds and removes server name from name if exists
        ...
        if core.BidInProgress then
            ...
            if cmd == "cancel" and MonDKP_DB.modes.mode ~= "Roll Based Bidding" then
                ...
                for i = 1, #Bids_Submitted do
                    -- !bid cancel will cancel their bid
            ...
            if not dkp then
                -- exits function if player is not on the DKP list
                ...
                if dkp then
                    if (cmd and cmd <= dkp) or (MonDKP_DB.modes.SubZeroBidding == true and dkp >= 0) or (MonDKP_DB.modes.SubZeroBidding == true and MonDKP_DB.modes.AllowNegativeBidders == true) or (mode == "Static Item Values" and dkp > 0 and (dkp > core.BiddingWindow.cost:GetNumber() or MonDKP_DB.modes.SubZeroBidding == true or MonDKP_DB.modes.costvalue == "Percent")) or ((mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Static") and not cmd) then
                        if (cmd and core.BiddingWindow.minBid and tonumber(core.BiddingWindow.minBid:GetNumber()) <= cmd) or mode == "Static Item Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Static") or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Minimum Bid" and cmd >= core.BiddingWindow.minBid:GetNumber()) then
                            for i = 1, #Bids_Submitted do
                                -- checks if a bid was submitted, removes last bid if it was
                            ...
                            if mode == "Minimum Bid Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Minimum Bid") then
                                ...
                                if MonDKP_DB.modes.DeclineLowerBids and Bids_Submitted[1] and cmd <= Bids_Submitted[1].bid then
                                    -- declines bids lower than highest bid
    ...
    elseif string.find(text, "!dkp") == 1 and core.IsOfficer == true then
        ...
        elseif cmd and cmd:gsub("%s+", "") ~= "nil" and cmd:gsub("%s+", "") ~= "" then
            -- allows command if it has content (removes empty spaces)
    ...
    ChatFrame_AddMessageEventFilter("CHAT_MSG_WHISPER_INFORM", function(self, event, msg, ...)
        -- suppresses outgoing whisper responses to limit spam
    ...
    ChatFrame_AddMessageEventFilter("CHAT_MSG_WHISPER", function(self, event, msg, ...)
        -- suppresses incoming whisper responses to limit spam
        if core.BidInProgress and MonDKP_DB.defaults.SupressTells then
...
function MonDKP:ToggleBidWindow(loot, lootIcon, itemName)
    ...
    if core.IsOfficer then
        ...
        if core.ModesWindow then
            core.ModesWindow:SetFrameLevel(6)
        end
        if MonDKP.UIConfig then
            MonDKP.UIConfig:SetFrameLevel(2)
        end
...
local function StartBidding()
    ...
    if mode == "Minimum Bid Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Minimum Bid") then
        ...
        if MonDKP_DB.defaults.AutoOpenBid then
            -- toggles bid window if option is set to
    ...
    if CurrItemForBid then
        ...
        if MonDKP_DB.modes.channels.raid then
            table.insert(channels, "/raid")
        end
        if MonDKP_DB.modes.channels.guild then
            table.insert(channels, "/guild")
        end
        if MonDKP_DB.modes.channels.whisper then
            table.insert(channels, "/whisper")
        end
...
local function ToggleTimerBtn(self)
    ...
    if timerToggle == 0 then
        ...
        if (mode == "Minimum Bid Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Minimum Bid")) and (not core.BiddingWindow.item:GetText() or core.BiddingWindow.minBid:GetText() == "") then
            MonDKP:Print(L["NOMINBIDORITEM"])
            return false
        end
        if (mode == "Static Item Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Static")) and (not core.BiddingWindow.item:GetText() or core.BiddingWindow.cost:GetText() == "") then
            MonDKP:Print(L["NOITEMORITEMCOST"])
            return false
        end
        if mode == "Roll Based Bidding" and (not core.BiddingWindow.item:GetText() or core.BiddingWindow.cost:GetText() == "") then
            MonDKP:Print(L["NOITEMORITEMCOST"])
            return false
        end
...
function ClearBidWindow()
    ...
    _G["MonDKPBiddingStartBiddingButton"]:SetScript("OnClick", function(self)
        ...
        if core.BiddingWindow.bidTimer then
            core.BiddingWindow.bidTimer:ClearFocus();
        end
        if core.BiddingWindow.minBid then
...
function MonDKP:BroadcastBidTimer(seconds, title, itemIcon)
    -- broadcasts timer and starts it natively
    local title = title;
...
function MonDKP_Register_ShiftClickLootWindowHook()
    -- hook function into LootFrame window. All slots on ElvUI. But only first 4 in default UI.
    local num = GetNumLootItems();

    if getglobal("ElvLootSlot1") then
        -- fixes hook for ElvUI loot frame
        for i = 1, num do
...
function MonDKP:StartBidTimer(seconds, title, itemIcon)
    ...
    if not extend then
        MonDKP.BidTimer:SetShown(not MonDKP.BidTimer:IsShown());
    end                    -- shows if not shown
    if core.BidInterface and core.BidInterface:IsShown() == false then
...
local function BidWindowCreateRow(parent, id)
    -- Create 3 buttons for each row in the list
...
function MonDKP:CreateBidWindow()
    ...
    if core.IsOfficer then
        ...
        f.boss:SetScript("OnEscapePressed", function(self)
            -- clears focus on esc
            self:HighlightText(0, 0)
            self:ClearFocus()
        end)
        f.boss:SetScript("OnEnterPressed", function(self)
            -- clears focus on esc
            self:HighlightText(0, 0)
            self:ClearFocus()
        end)
        f.boss:SetScript("OnTabPressed", function(self)
            -- clears focus on esc
            self:HighlightText(0, 0)
            self:ClearFocus()
        end)
        ...
        if mode == "Minimum Bid Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Minimum Bid") then
            ...
            f.minBid:SetScript("OnEscapePressed", function(self)
                -- clears focus on esc
                self:ClearFocus()
            end)
        ...
        f.bidTimer:SetScript("OnEscapePressed", function(self)
            -- clears focus on esc
            self:ClearFocus()
        end)
        f.bidTimer:SetScript("OnEscapePressed", function(self)
        ...
        f.cost:SetScript("OnEscapePressed", function(self)
            -- clears focus on esc
            self:ClearFocus()
        end)
        ...
        f.StartBidding:SetSize(90, 25)
        f.StartBidding:SetScript("OnClick", function()
