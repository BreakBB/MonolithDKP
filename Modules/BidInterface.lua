...
function BidInterface_Update()
    ...
    if not core.BidInterface.bidTable:IsShown() then
        return
    end
    ...
    for i = 1, showRows do
        ...
        if dkp_total then
            ...
            if createProfile then
                ...
                -- if unable to create profile, feeds grey color
                c = { r = "aa", g = "aa", b = "aa" }
            end
        end
...
function MonDKP:BidInterface_Toggle()
    ...
    if core.BidInterface:IsShown() then
        core.BidInterface:Hide();
    end
    if MonDKP.BidTimer.OpenBid and MonDKP.BidTimer.OpenBid:IsShown() then
        MonDKP.BidTimer.OpenBid:Hide()
    end
...
function MonDKP:BidInterface_Create()
    ...
    f.Bid:SetScript("OnEscapePressed", function(self)
        -- clears focus on esc
        self:ClearFocus()
    end)
