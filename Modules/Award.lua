...
local function AwardItem(player, cost, boss, zone, loot, reassign)
    ...
    if core.IsOfficer then
        ...
        if core.BiddingWindow and core.BiddingWindow:IsShown() then
            -- runs below if award is through bidding window (update minbids and zerosum bank)
...
local function AwardConfirm_Create()
    ...
    f.cost:SetScript("OnTabPressed", function(self)
        -- clears focus on tab
        self:ClearFocus()
    end)
    f.cost:SetScript("OnEnterPressed", function(self)
        -- clears focus on enter
        self:ClearFocus()
    end)
...
function MonDKP:AwardConfirm(player, cost, boss, zone, loot, reassign)
    ...
    UIDropDownMenu_Initialize(core.AwardConfirm.bossDropDown, function(self, level, menuList)
        -- BOSS dropdown
        ...
        for k, v in pairs(core.LastKilledNPC) do
            -- eliminates duplicate zones
    ...
    UIDropDownMenu_Initialize(core.AwardConfirm.zoneDropDown, function(self, level, menuList)
        -- ZONE dropdown
        ...
        for k, v in pairs(core.RecentZones) do
            -- eliminates duplicate zones
    ...
    function core.AwardConfirm.player:SetValue(newValue, arg2)
    ...
    function core.AwardConfirm.bossDropDown:SetValue(newValue)
        ---- BOSS dropdown function
        UIDropDownMenu_SetText(core.AwardConfirm.bossDropDown, newValue)
        curBoss = newValue;
        CloseDropDownMenus()
    end

    function core.AwardConfirm.zoneDropDown:SetValue(newValue)
        ---- ZONE dropdown function
        UIDropDownMenu_SetText(core.AwardConfirm.zoneDropDown, newValue)
        curZone = newValue;
        CloseDropDownMenus()
    end

    core.AwardConfirm.yesButton:SetScript("OnClick", function()
        -- Run when "Yes" is clicked
        if not player then
    ...
    core.AwardConfirm.noButton:SetScript("OnClick", function()
        -- Run when "No" is clicked
        PlaySound(851)
        core.AwardConfirm:SetShown(false)
    end)
end