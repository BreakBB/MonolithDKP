...
function MonDKP:SortDKPHistoryTable()
    -- sorts the DKP History Table by date/time
...
function DKPHistoryFilterBox_Create()
    ...
    function filterDropdown:FilterSetValue(newValue, arg2)
        if curfilterName ~= newValue then
            curfilterName = newValue
        else
            curfilterName = nil
        end
...
local function MonDKPDeleteDKPEntry(index, timestamp, item)
    -- index = entry index (Roeshambo-1), item = # of the entry on DKP History tab; may be different than the key of DKPHistory if hidden fields exist
    ...
    if strfind(reason_header, L["OTHER"] .. "- ") then
        reason_header = reason_header:gsub(L["OTHER"] .. " -- ", "")
    end
    ...
    StaticPopupDialogs["CONFIRM_DELETE"] = {
        ...
        OnAccept = function()
            ...
            if search then
                ...
                if strfind(MonDKP_DKPHistory[search[1][1]].dkp, "%-%d*%.?%d+%%") then
...
function MonDKP:DKPHistory_Update(reset)
    ...
    if not MonDKP.UIConfig:IsShown() then
        -- prevents history update from firing if the DKP window is not opened (eliminate lag). Update run when opened
        return ;
    end
    ...
    if currentLength > #DKPHistory then
        currentLength = #DKPHistory
    end
    ...
    DKPHistTimer:SetScript("OnUpdate", function(self, elapsed)
        ...
        if HistTimer > 0.001 and j <= currentLength and not processing then
            ...
            if player_table[1] ~= nil and #player_table > 1 then
                -- removes last entry in table which ends up being nil, which creates an additional comma at the end of the string
        ...
        elseif j > currentLength then
            ...
            elseif MonDKP.ConfigTab6.loadMoreBtn and currentRow < #DKPHistory then
                if (#DKPHistory - currentRow) < btnText then
                    btnText = (#DKPHistory - currentRow)
                end
                MonDKP.ConfigTab6.loadMoreBtn:SetText(string.format(L["LOAD50MORE"], btnText) .. "...")
                MonDKP.ConfigTab6.loadMoreBtn:SetPoint("TOP", MonDKP.ConfigTab6.history[currentRow], "BOTTOM", 0, -10);
