...
local function RightClickLootMenu(self, index)
    -- called by right click function on ~201 row:SetScript
    local search = MonDKP:Table_Search(MonDKP_Loot, index, "index")
    menu = {
...
function MonDKP:LootHistory_Update(filter)
    -- if "filter" is included in call, runs set assigned for when a filter is selected in dropdown.
    ...
    LootHistTimer:SetScript("OnUpdate", function(self, elapsed)
        ...
        if LootTimer > 0.001 and j <= CurrentLimit and not processing then
            ...
            if tonumber(LootTable[i].cost) < 0 then
                lootCost = tonumber(LootTable[i].cost) * -1
            else
                lootCost = tonumber(LootTable[i].cost)
            end
            ...
            MonDKP.ConfigTab5.lootFrame[i]:SetScript("OnEnter", function(self)
                ...
                if LootTable[i].bids or LootTable[i].dkp or LootTable[i].rolls then
                    -- displays bids/rolls/dkp values if "Log Bids" checked in modes
                    local path;
