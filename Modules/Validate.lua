...
function MonDKP_TableCompare(t1, t2)
    -- compares two tables. returns true if all keys and values match
    local ty1 = type(t1)
    local ty2 = type(t2)
    ...
    ValidateTimer:SetScript("OnUpdate", function(self, elapsed)
        ...
        if timer > 0.01 and i <= #MonDKP_DKPHistory and not processing then
            ...
            if MonDKP_DKPHistory[i].deletes then
                -- adds deltedby index to field if it was received after a delete entry was received but was sent by someone that did not have the delete entry
