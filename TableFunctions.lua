...
local function RightClickMenu(self)
    ...
    for i = 1, #core.classes do
        -- create Filter selections in context menu
...
local function CreateRow(parent, id)
...
function DKPTable_Update()
    if not MonDKP.UIConfig:IsShown() then
        -- does not update list if DKP window is closed. Gets done when /dkp is used anyway.
        return ;
    end

    if core.CurView == "limited" then
        -- recreates WorkingTable if in limited view (view raid, core raiders etc)
        local tempTable = {}
    ...
    for i = 1, core.TableNumRows do
        -- hide all rows before displaying them 1 by 1 as they show values
    ...
    if #core.WorkingTable == 0 then
        -- Displays "No Entries Returned" if the result of filter combinations yields an empty table
        ...
        if MonDKP.DKPTable.Rows[1].DKPInfo[3].rollrange then
            MonDKP.DKPTable.Rows[1].DKPInfo[3].rollrange:SetText("")
        end
