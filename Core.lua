...
function MonDKP:CheckOfficer()
    -- checks if user is an officer IF core.IsOfficer is empty. Use before checks against core.IsOfficer
...
function MonDKP:GetGuildRankGroup(index)
    -- returns all members within a specific rank index as well as their index in the guild list (for use with GuildRosterSetPublicNote(index, "msg") and GuildRosterSetOfficerNote)
...
function MonDKP:PurgeLootHistory()
    -- cleans old loot history beyond history limit to reduce native system load
...
function MonDKP:PurgeDKPHistory()
    -- purges old entries and stores relevant data in each users MonDKP_Archive entry (dkp, lifetime spent, and lifetime gained)
        ...
        while #MonDKP_DKPHistory > limit do
            ...
            for i = 1, #players do
                if not MonDKP_Archive[players[i]] then
                    ...
                    if ((dkp[i] > 0 and not path.deletes) or (dkp[i] < 0 and path.deletes)) and not strfind(path.dkp, "%-%d*%.?%d+%%") then
                        --lifetime gained if dkp addition and not a delete entry, dkp decrease and IS a delete entry
...
function MonDKP:BroadcastTimer(seconds, ...)
    -- broadcasts timer and starts it natively
    if IsInRaid() and core.IsOfficer == true then
        local title = ...;
        if not tonumber(seconds) then
            -- cancels the function if the command was entered improperly (eg. no number for time)
...
function MonDKP:StartTimer(seconds, ...)
    ...
    if MonDKP.BidTimer:IsShown() == false then
        -- terminates function if hiding timer
        return ;
    end
...
function MonDKP:StatusVerify_Update()
    if (MonDKP.UIConfig and not MonDKP.UIConfig:IsShown()) or (#MonDKP_DKPHistory == 0 and #MonDKP_Loot == 0) then
    ...
    if IsInGuild() and core.Initialized then
        ...
        if MonDKP_Loot.seed and MonDKP_DKPHistory.seed and strfind(MonDKP_Loot.seed, "-") and strfind(MonDKP_DKPHistory.seed, "-") then
            ...
            if not search_dkp then
                ...
                if (date1 and tonumber(date1) < (time() - 1209600)) or not MonDKP:ValidateSender(officer1) then
                    -- does not consider if claimed entry was made more than two weeks ago or name is not an officer
                    core.OOD = false
                else
            ...
            if not search_loot then
                ...
                if (date2 and tonumber(date2) < (time() - 1209600)) or not MonDKP:ValidateSender(officer2) then
                    -- does not consider if claimed entry was made more than two weeks ago or name is not an officer
                    core.OOD = false
                else
...
function MonDKP:TableStrFind(tar, val, field)
    -- same function as above, but searches values that contain the searched string rather than exact string matches
...
function MonDKP:DKPTable_Set(tar, field, value, loot)
    -- updates field with value where tar is found (IE: MonDKP:DKPTable_Set("Roeshambo", "dkp", 10) adds 10 dkp to user Roeshambo). loot = true/false if it's to alter lifetime_spent
