...
function MonDKPFilterChecks(self)
    -- sets/unsets check boxes in conjunction with "All" button, then runs MonDKP:FilterDKPTable() above
    ...
    for i = 1, 8 do
        -- checks all boxes to see if all are checked, if so, checks "All" as well
...
function MonDKP:ConfigMenuTabs()
    ...
    for i = 1, 10 do
        ...
        if i == 9 then
            ...
            for k, v in pairs(core.classes) do
                -- sets core.classFiltered table with all values
                if (MonDKP.ConfigTab1.checkBtn[k]:GetChecked() == true) then
                    core.classFiltered[v] = true;
                else
                    core.classFiltered[v] = false;
